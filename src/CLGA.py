...
class Metacl(Module):
    ...
    def attack(self):
        ...
        # save three poisoned adj when the perturbation rate reaches 1%, 5%, 10%, 15%, 20%
        while len(perturbed_edges) < int(0.20 * num_total_edges):
            ...
            if len(perturbed_edges) == int(0.10 * num_total_edges) or \
                    len(perturbed_edges) == int(0.10 * num_total_edges)-1 or \
                    len(perturbed_edges) == int(0.10 * num_total_edges)-2:
                output_adj = adj.to(device)
                pkl.dump(output_adj.to(torch.device('cpu')), open('poisoned_adj/%s_CLGA_0.100000_adj.pkl' % args.dataset, 'wb'))
                print('---10% perturbed adjacency matrix saved---')
            if len(perturbed_edges) == int(0.15 * num_total_edges) or \
                    len(perturbed_edges) == int(0.15 * num_total_edges)-1 or \
                    len(perturbed_edges) == int(0.15 * num_total_edges)-2:
                output_adj = adj.to(device)
                pkl.dump(output_adj.to(torch.device('cpu')), open('poisoned_adj/%s_CLGA_0.150000_adj.pkl' % args.dataset, 'wb'))
                print('---15% perturbed adjacency matrix saved---')
            start = time.time()
            self.inner_train()
            ...
            print('Perturbing edges: %d/%d. Finished in %.2fs' % (len(perturbed_edges)/2, int(0.20 * num_total_edges)/2, end-start))
...
if __name__ == '__main__':
    ...
    pkl.dump(poisoned_adj.to(torch.device('cpu')), open('poisoned_adj/%s_CLGA_0.200000_adj.pkl' % args.dataset, 'wb'))
